...
importScripts('VexWarp/dsp.js', 'VexWarp/tools.js')
...
//var TimeStretcher = (function() {
  ...
  //  TimeStretcher.prototype.init(options);
  //}

  function onProgress(details, message) {
    if (!message) message = "";
    Module.onProgress(details, message);
  }
  ...
  class TimeStretcher {
    ...
    init(options) {
      ...
      Module.merge(this.options, options);
    ...
    resize(size) {
      ...
      this.resampled_buffer = Module.interpolateArray(buffer, newBuffer, size);
      return this;
    }
  }

  Module['VexWarpStretch'] = TimeStretcher;
